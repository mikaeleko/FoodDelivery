...
public class DBAdapter extends SQLiteOpenHelper {
	//private static String DB_PATH = "/data/data/freaktemplate.storage/databases/";
	...
	public Cursor getChapter(String id) throws SQLException {
		Cursor cur;
		cur = myDataBase.rawQuery("select * from hindibible where Book=" + id + " and Chapter=1", null);

		// cur =
		// myDataBase.rawQuery("select * from hindibible where Book="+id+" and
		// Chapter="+id+";",
		// null);
		return cur;
	...
	public Cursor getChapter1(String name, int id) throws SQLException {
		Cursor cur;
		cur = myDataBase.rawQuery("select * from hindibible where Book=" + name + " and Chapter=" + id + "", null);
	...
	public Cursor getFavourite() throws SQLException {
		Cursor cur;
		cur = myDataBase.rawQuery("select * from Dictionary where isFav=1;", null);
		return cur;
	...
	public Cursor getFavourite1() throws SQLException {
		Cursor cur;
		cur = myDataBase.rawQuery("select * from hindibible where IsFav=1;", null);
		return cur;
	...
	public Cursor getquestion(int id) throws SQLException {
		...
		Log.d("QUERY", "" + query);
		cur = myDataBase.rawQuery("select * from bibleqtable where SlNo=" + id + ";", null);
	...
	public Cursor getFrench(String id) throws SQLException {
		// Cursor
		// cur=myDataBase.rawQuery("select * from Dictionary where
		// English='"+name+"';",null);
		Cursor cur;
		cur = myDataBase.rawQuery("select * from Dictionary where Id=" + id + ";", null);
		return cur;
	}
	...
	public Cursor getFvrt(int str) throws SQLException {
		...
		Cursor cur;
		cur = myDataBase.rawQuery("select * from hindibible where ID=" + str + ";", null);
	...
	public Cursor getAll(String name) throws SQLException {
		...
		if (name.equals("*")) {
			...
			// null);
			cur = myDataBase.rawQuery("select English,Hindi from Dictionary order by random() limit 1 ;", null);
		} else {
	...
	public Cursor getday(String name) throws SQLException {
		...
		if (name.equals("*")) {
			// cur = myDataBase.rawQuery("select English from Dictionary ;",
			// null);
			cur = myDataBase.rawQuery("select NKJ from hindibible order by random() limit 1 ;", null);
		} else {
			cur = myDataBase.rawQuery("select * from Dictionary ;", null);
	...
	public Cursor getnotes1(String name) throws SQLException {
		...
		Cursor cur;
		cur = myDataBase.rawQuery("select * from hindibible where Version=" + name + ";", null);
		return cur;
	}
}
